...
function App() {
  ...
  const fetchMaterias = async () => {
    try {
      const response = await fetch('http://localhost:30081/projects');
      if (!response.ok) {
  ...
  const handleAgregarMateria = async (nombre, imagenURL) => {
    try {
      ...
      const response = await axios.post('http://localhost:30081/projects', nuevaMateria);
  ...
  const handleEliminarMateria = async (materiaId) => {
    try {
      ...
      if (confirmacion) {
        await axios.delete(`http://localhost:30081/projects/${materiaId}`);
  ...
  const handleEditarMateria = async (materiaId, nuevoNombre) => {
    try {
      ...
      await axios.patch(`http://localhost:30081/projects/${materiaId}`, materiaActualizada);
