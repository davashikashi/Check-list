...
function Tarea({ materia }) {
  ...
  const fetchTareas = async () => {
    try {
      const response = await fetch(`https://3jt241sk-3000.use.devtunnels.ms/tasks/project/${materia.id}`);
      if (!response.ok) {
  ...
  const createTarea = async (nuevaTarea) => {
    try {
      const response = await axios.post('https://3jt241sk-3000.use.devtunnels.ms/tasks', {
        ...nuevaTarea,
  ...
  const updateTarea = async (tareaActualizada) => {
    try {
      ...
      await axios.patch(`https://3jt241sk-3000.use.devtunnels.ms/tasks/${tareaActualizada.id}`, tareaUpdated);
      fetchTareas();
    } catch (error) {
  ...
  const eliminarTarea = async (taskId) => {
    try {
      ...
      if (confirmacion) {
        await axios.delete(`https://3jt241sk-3000.use.devtunnels.ms/tasks/${taskId}`);
